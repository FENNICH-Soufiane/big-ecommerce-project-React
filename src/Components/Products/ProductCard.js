  ...
  return (
    <Col xs="6" sm="6" md="4" lg="3" className="d-flex">
      ...
      <Card
        ...
        <Link to={`/products/${item._id}`} onClick={window.location.reload} style={{ textDecoration: 'none' }}>
                ...
                <div className="card-price">
                  {
                    item.priceAfterDiscount >= 1 ?
                      (<><sapn>{item.priceAfterDiscount}</sapn><span className="text-decoration-line-through fs-6">{item.price}</span></>)
                      : item.price
                  }
