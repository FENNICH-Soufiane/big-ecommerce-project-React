  ...
  return (
    <Container className='rate-container'>
      ...
      {allReview && allReview.data ? (allReview.data.map((review, index) => {
        return (
          <RateItem review={review} key={index} />
        )
      })) : <h6>لا يوجد تقييمات الان</h6>}

      {
        allReview.paginationResult && allReview.paginationResult.numberOfPages >= 2 ? (<Pagination pageCount={allReview.paginationResult ? allReview.paginationResult.numberOfPages : 0}
